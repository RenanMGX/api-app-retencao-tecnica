...
class EncontrarBotoes:
    ...
    def find_per_attribute(self, *, attribute:str, target:str, wait:int|float=0) -> WebElement:
        sleep(wait)
    ...
    def find_per_text(self, target:str, *, type:Literal['in', 'equal']='equal', wait:int|float=0) -> WebElement:
        sleep(wait)
        for button in self.buttons:
            if type == 'equal':
